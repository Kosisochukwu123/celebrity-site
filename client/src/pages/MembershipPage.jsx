...
const TIERS = [
  {
    ...
    stars: 2,
    benefits: [
      { name: 'Member Newsletter', desc: 'Monthly advocacy updates', badge: null, icon: 'newsletter' },
      { name: 'Digital Card', desc: 'Official digital ID', badge: 'Free', icon: 'card' },
      { name: 'Community Forum', desc: 'Connect with members', badge: null, icon: 'forum' },
      { name: 'Impact Reports', desc: 'Annual PDF reports', badge: null, icon: 'report' },
      { name: 'Event Updates', desc: 'Early event notices', badge: null, icon: 'event' },
      { name: 'Online Screenings', desc: 'Select documentaries', badge: null, icon: 'film' },
    ],
    compareCol: 0,
  },
  {
    ...
    stars: 4,
    benefits: [
      { name: 'All Silver', desc: 'Everything in Silver', badge: null, icon: 'check' },
      { name: 'Physical Card', desc: 'Collector\'s edition card', badge: 'Free', icon: 'card' },
      { name: 'Priority Events', desc: 'First-access registration', badge: 'Excl', icon: 'star' },
      { name: 'Full Library', desc: 'All documentaries', badge: null, icon: 'film' },
      { name: 'Private Gallery', desc: 'Behind-the-scenes archive', badge: 'Excl', icon: 'gallery' },
      { name: 'Quarterly Call', desc: 'Live Q&A with team', badge: 'Excl', icon: 'call' },
    ],
    compareCol: 1,
  },
  {
    ...
    stars: 5,
    benefits: [
      { name: 'All Gold', desc: 'Everything in Gold', badge: null, icon: 'check' },
      { name: 'VIP Events', desc: 'Galas & private dinners', badge: 'Excl', icon: 'vip' },
      { name: 'Name on Wall', desc: 'Foundation donor wall', badge: 'Excl', icon: 'honor' },
      { name: 'Meet & Greet', desc: 'Annual private session', badge: 'Excl', icon: 'star' },
      { name: 'Legal Fund', desc: 'Priority case support', badge: 'Excl', icon: 'legal' },
      { name: 'Founding Plaque', desc: 'Personalised keepsake', badge: 'Free', icon: 'award' },
    ],
    compareCol: 2,
  },
];
...
function BenefitIcon({ type, color }) {
  ...
  const icons = {
    newsletter: <><rect x="3" y="5" width="14" height="14" rx="2" {...s}/><path d="M7 9h6M7 12h4" {...s}/></>,
    card:       <><rect x="2" y="5" width="16" height="12" rx="2" {...s}/><path d="M2 9h16" {...s}/></>,
    forum:      <><path d="M4 4h12v10H4z" {...s}/><path d="M8 18l2-4h6l2 4" {...s}/></>,
    report:     <><path d="M6 2h8l4 4v14H6V2z" {...s}/><path d="M14 2v4h4M9 10h6M9 13h6M9 16h4" {...s}/></>,
    event:      <><rect x="3" y="4" width="14" height="14" rx="2" {...s}/><path d="M7 2v2M13 2v2M3 10h14" {...s}/></>,
    film:       <><rect x="2" y="5" width="16" height="12" rx="2" {...s}/><path d="M2 9h16M2 13h16M6 5v2M10 5v2M14 5v2M6 13v2M10 13v2M14 13v2" {...s}/></>,
    check:      <><path d="M4 10l5 5 8-8" {...s}/></>,
    star:       <><path d="M10 2l2.4 4.8 5.3.8-3.8 3.7.9 5.2L10 14l-4.8 2.5.9-5.2L2.3 7.6l5.3-.8L10 2z" {...s}/></>,
    gallery:    <><rect x="3" y="3" width="14" height="14" rx="2" {...s}/><circle cx="7.5" cy="7.5" r="1.5" {...s}/><path d="M3 14l4-4 3 3 2-2 5 5" {...s}/></>,
    call:       <><path d="M5 4h4l2 5-2.5 1.5a11 11 0 005 5L15 13l5 2v4a2 2 0 01-2 2A16 16 0 013 5a2 2 0 012-1z" {...s}/></>,
    vip:        <><path d="M3 7l7 10 7-10H3z" {...s}/><path d="M3 7h14" {...s}/></>,
    honor:      <><circle cx="10" cy="8" r="5" {...s}/><path d="M6.5 14.5L5 19l5-2 5 2-1.5-4.5" {...s}/></>,
    legal:      <><path d="M10 2l1.5 4.5H16L12 9.5l1.5 4.5L10 11l-3.5 3 1.5-4.5L4 7h4.5L10 2z" {...s}/></>,
    award:      <><path d="M6 9H4a2 2 0 00-2 2v7h16v-7a2 2 0 00-2-2h-2M10 3v6M7 6l3-3 3 3" {...s}/></>,
  };
  return (
...
export default function MembershipPage() {
  ...
  const iconColor = active === 0 ? '#C8C8C8' : active === 1 ? '#C9A84C' : '#E8C97A';
  ...
  const handlePurchase = () => {
    if (!user) {
      ...
      alert(`Purchasing ${tier.name} membership — payment integration coming soon!`);
    }
  };
  ...
  return (
    <div className="membership-page">
      ...
      <header className="mem-header">
        ...
        <h1 className="mem-title">Choose Your<br /><em>Membership</em></h1>
      ...
      <section className="mem-carousel-section">
        ...
        >
          <div className="mem-carousel-track" ref={trackRef}
                  ...
                  <div className="mem-card-inner">
                    ...
                    <div className="mem-card-bottom">
                      <div className="mem-card-price">
                        ...
                        <span className="mem-card-price-period">{t.period}</span>
                      </div>
        ...
        <div className="mem-arrows">
          <button className="mem-arrow" onClick={() => goTo(active - 1)} disabled={active === 0}>←</button>
          <button className="mem-arrow" onClick={() => goTo(active + 1)} disabled={active === TIERS.length - 1}>→</button>
        </div>
      </section>
        ...
        <div className="mem-benefits-grid">
          {tier.benefits.map((b) => (
            <div key={b.name} className="mem-benefit-item">
              {b.badge && (
                <span className={`mem-benefit-badge mem-benefit-badge--${b.badge === 'Free' ? 'free' : 'excl'}`}>
                  {b.badge}
                </span>
              )}
        ...
        <table className="mem-compare-table">
          <thead>
            <tr>
              <th>Benefit</th>
              {TIERS.map((t, i) => (
                <th key={t.id} className={i === active ? 'active-col' : ''}
              ...
              <tr key={row.label}>
                ...
                {row.vals.map((v, i) => (
                  <td key={i} className={i === active ? 'active-col' : ''}>
                    {v ? <span className="mem-check">✦</span> : <span className="mem-cross">—</span>}
                  </td>
                ))}
              </tr>
            ))}
