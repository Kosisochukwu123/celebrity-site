...
const API = 'http://localhost:5000' || import.meta.env.VITE_BACKEND_URL || '';
const imgSrc = (url) => (!url ? '' : url.startsWith('http') ? url : `${API}${url}`);
...
export default function CauseGalleryManager() {
  ...
  useEffect(() => { loadImages(activeCause); }, [activeCause]);
  ...
  const uploadFiles = async (files) => {
    ...
    try {
      ...
      const res = await axios.post(`/api/admin/content/${activeCause}/gallery`, data, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      setImages(res.data?.images || []);
      setMsg({ type: 'success', text: `✓ ${files.length} image${files.length > 1 ? 's' : ''} uploaded.` });
    } catch (err) {
  ...
  const deleteImage = async (imageId) => {
    ...
    try {
      await axios.delete(`/api/admin/content/${activeCause}/gallery/${imageId}`);
  ...
  const saveCaption = async (imageId) => {
    try {
      const res = await axios.patch(
        `/api/admin/content/${activeCause}/gallery/${imageId}`,
  ...
  return (
    <div className="gm-wrap">
      ...
      >
        ...
        <p className="gm-dropzone-hint">jpg, png, webp, gif · max 8 MB per file · up to 10 at once</p>
      </div>
            ...
            {images.map(img => (
              <div key={img._id} className="gm-item">
                <div className="gm-item-img-wrap">
                  <img src={imgSrc(img.url)} alt={img.caption || 'Gallery image'} />
                  <button
                ...
                {editingId === img._id ? (
                  <div className="gm-caption-edit">
                    <input
                      ...
                      autoFocus
                      onKeyDown={e => { if (e.key === 'Enter') saveCaption(img._id); if (e.key === 'Escape') setEditingId(null); }}
                    />
                    <div className="gm-caption-actions">
                      <button className="gm-caption-save" onClick={() => saveCaption(img._id)}>Save</button>
                      <button className="gm-caption-cancel" onClick={() => setEditingId(null)}>Cancel</button>
                    </div>
                  </div>
                ) : (
                  <div className="gm-caption-display" onClick={() => startEdit(img)}>
                    {img.caption
