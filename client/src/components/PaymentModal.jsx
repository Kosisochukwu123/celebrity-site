...
const COINS = [
  { id: 'btc',  label: 'Bitcoin',  symbol: 'BTC',  icon: '₿', settingKey: 'wallet_btc'  },
  { id: 'eth',  label: 'Ethereum', symbol: 'ETH',  icon: 'Ξ', settingKey: 'wallet_eth'  },
...
export default function PaymentModal({ tier, onClose }) {
  ...
  const canProceedStep2 = () => {
    ...
    if (method === 'giftcard') return giftImage !== null && giftAmount.trim() !== '';
    return false;
  };
      ...
      <div className="pm-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="pm-header">
          <div className="pm-header-tier">
            ...
            <span className="pm-tier-price">{tier.price}<span>/yr</span></span>
          </div>
          ...
          {!submitted && (
            <div className="pm-steps">
              {STEPS.map((s, i) => (
                <div key={s} className={`pm-step ${i <= step ? 'active' : ''} ${i < step ? 'done' : ''}`}>
        ...
        <div className="pm-body">
          ...
          {step === 0 && (
            <div className="pm-section">
              ...
              <p className="pm-section-sub">Choose your preferred payment method. Both are manually verified within 24–48 hours.</p>
              ...
              <div className="pm-method-grid">
                ...
                >
                  <div className="pm-method-icon pm-method-icon--crypto">
                    <svg viewBox="0 0 40 40" fill="none"><circle cx="20" cy="20" r="18" stroke="currentColor" strokeWidth="1.5"/><path d="M14 13h7a4 4 0 010 8h-7v-8zM14 21h8a4 4 0 010 8h-8v-8z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/><line x1="18" y1="11" x2="18" y2="13" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/><line x1="22" y1="11" x2="22" y2="13" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/><line x1="18" y1="29" x2="18" y2="31" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/><line x1="22" y1="29" x2="22" y2="31" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>
                  </div>
                  <div className="pm-method-text">
                    <strong>Cryptocurrency</strong>
                    <span>Bitcoin, Ethereum, USDT</span>
                  </div>
                  <div className="pm-method-check">{method === 'crypto' ? '✓' : ''}</div>
                </button>
                ...
                >
                  <div className="pm-method-icon pm-method-icon--gift">
                    <svg viewBox="0 0 40 40" fill="none"><rect x="6" y="16" width="28" height="20" rx="2" stroke="currentColor" strokeWidth="1.5"/><path d="M6 22h28" stroke="currentColor" strokeWidth="1.5"/><path d="M20 16V36" stroke="currentColor" strokeWidth="1.5"/><path d="M20 16c0 0-3-7 0-7s3 7 0 7z" stroke="currentColor" strokeWidth="1.5"/><path d="M20 16c0 0 3-7 0-7s-3 7 0 7z" stroke="currentColor" strokeWidth="1.5"/></svg>
                  </div>
                  <div className="pm-method-text">
                    <strong>Gift Card</strong>
                    <span>Amazon, Apple, Google Play & more</span>
                  </div>
                  <div className="pm-method-check">{method === 'giftcard' ? '✓' : ''}</div>
                </button>
              </div>

              <div className="pm-security-note">
                <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" width="14" height="14"><path d="M10 2L3 5v5c0 4.4 3 8.1 7 9 4-0.9 7-4.6 7-9V5l-7-3z"/></svg>
                Payments are reviewed manually. Your membership activates within 24–48 hours of approval.
              </div>
            ...
            <div className="pm-section">
              {method === 'crypto' && (
                <>
                  ...
                  <p className="pm-section-sub">Select a coin, send the exact amount to the address below, then paste your transaction hash.</p>
                  ...
                  <div className="pm-amount-row">
                    ...
                    <span className="pm-amount-value">{tier.price} USD equivalent in {selectedCoin.symbol}</span>
                  </div>
                  ...
                  <div className="pm-wallet-card">
                    <div className="pm-wallet-left">
                      <p className="pm-wallet-network">
                        {coin === 'usdt' ? `Network: ${usdtNetwork}` : `${selectedCoin.label} (${selectedCoin.symbol})`}
                      </p>
                      {walletAddress
                        ? <>
                            <p className="pm-wallet-label">Send to this address:</p>
                            <p className="pm-wallet-address">{walletAddress}</p>
                            <button className={`pm-copy-btn ${copied ? 'copied' : ''}`} onClick={copyAddress}>
                              {copied ? '✓ Copied!' : 'Copy Address'}
                            </button>
                          </>
                        : <p className="pm-wallet-unavailable">Wallet address not configured yet. Contact support.</p>
                      }
                    </div>
                    {walletAddress && (
                  ...
                  <div className="pm-warning">
                    ⚠️ Send only <strong>{selectedCoin.symbol}</strong> to this address.
                    {coin === 'usdt' && <> Use <strong>{usdtNetwork}</strong> network only.</>} Sending the wrong coin will result in permanent loss.
                  </div>
                  ...
                  <div className="pm-field">
                    ...
                    />
                    <span className="pm-hint">Found in your wallet app after the transaction is sent.</span>
                  </div>
                </>
              )}
              ...
              {method === 'giftcard' && (
                <>
                  ...
                  <p className="pm-section-sub">Upload a clear photo of your gift card showing the code and amount. Our team verifies within 24–48 hours.</p>

                  <div className="pm-gift-grid">
                    <div className="pm-field">
                      <label className="pm-label">Gift Card Brand *</label>
                      <select className="pm-input pm-select" value={giftBrand} onChange={e => setGiftBrand(e.target.value)}>
                        {GIFT_BRANDS.map(b => <option key={b} value={b}>{b}</option>)}
                      </select>
                    </div>
                    <div className="pm-field">
                  ...
                  {/* Image upload */}
                  <div className="pm-field">
                    <label className="pm-label">Card Photo * <span className="pm-label-hint">(show front + scratch code clearly)</span></label>
                    <input
                      ref={fileRef}
                      type="file"
                    ...
                    {giftPreview
                      ? (
                        <div className="pm-gift-preview">
                          ...
                          <button type="button" className="pm-gift-reupload" onClick={() => fileRef.current?.click()}>
                            Change photo
                          </button>
                        </div>
                      )
                      : (
                        <div className="pm-gift-dropzone" onClick={() => fileRef.current?.click()}>
                          <svg viewBox="0 0 40 40" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" width="36" height="36"><rect x="4" y="8" width="32" height="24" rx="3"/><circle cx="14" cy="17" r="3"/><path d="M4 28l8-8 5 5 4-4 11 11"/></svg>
                  ...
                  <div className="pm-gift-tips">
                    <p className="pm-gift-tips-title">Tips for a successful submission:</p>
                    <ul>
                      <li>Make sure the card code and PIN are clearly visible</li>
              ...
              <div className="pm-btn-row">
                <button className="pm-back-btn" onClick={() => setStep(0)}>← Back</button>
                <button
          ...
          {step === 2 && submitted && (
            <div className="pm-success">
              ...
              <p className="pm-success-body">
                Your <strong>{tier.name} membership</strong> payment has been received and is now under review.
              </p>
              ...
              <div className="pm-success-timeline">
                ...
                <div className="pm-timeline-item">
                  ...
                  <div>
                    ...
                    <span>Our team reviews your payment — typically within 24 hours</span>
                  </div>
                </div>
                <div className="pm-timeline-item">
                  <div className="pm-timeline-dot pm-timeline-dot--pending" />
                  <div>
                    ...
                    <span>You receive full access within 24–48 hours of approval</span>
                  </div>
                </div>
              </div>

              <div className="pm-success-note">
                <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" width="14" height="14"><circle cx="10" cy="10" r="8"/><line x1="10" y1="7" x2="10" y2="10"/><line x1="10" y1="13" x2="10" y2="13.5"/></svg>
                If you have questions, use the live chat at the bottom right of the screen. Reference your payment method: <strong>{method === 'crypto' ? `${selectedCoin?.symbol} · ${txHash.slice(0,12)}...` : `${giftBrand} Gift Card`}</strong>
              </div>
