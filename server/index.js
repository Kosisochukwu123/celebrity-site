...
io.on('connection', (socket) => {
  ...
  if (user.role !== 'admin') {
    ...
    socket.on('user:firstOpen', async () => {
      const existing = await Message.countDocuments({ conversationId: user._id.toString() });
      if (existing === 0) {
        const botMsg = await Message.create({
          conversationId: user._id.toString(),
          senderId: user._id, senderName: 'Support', role: 'bot',
    ...
    socket.on('user:message', async (text) => {
      ...
      const msg = await Message.create({
        conversationId: user._id.toString(),
        senderId: user._id, senderName: user.name, role: 'user', text: text.trim(),
      });
      socket.emit('chat:message', msg);
      io.to(`admin_watch_${user._id}`).emit('chat:message', msg);
      io.to('admins').emit('chat:newMessage', {
        conversationId: user._id.toString(),
        senderName: user.name, text: text.trim(), time: msg.createdAt,
      });
    });
  }
  ...
  if (user.role === 'admin') {
    ...
    socket.on('admin:watchConversation', (conversationId) => {
      if (adminWatching[socket.id]) socket.leave(`admin_watch_${adminWatching[socket.id]}`);
    ...
    socket.on('admin:reply', async ({ conversationId, text }) => {
      ...
      const msg = await Message.create({
        conversationId, senderId: user._id, senderName: 'Support Team', role: 'admin', text: text.trim(),
      });
  ...
  socket.on('disconnect', () => { if (adminWatching[socket.id]) delete adminWatching[socket.id]; });
});
